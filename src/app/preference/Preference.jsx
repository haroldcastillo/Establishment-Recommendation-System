...
export default function Preference({ children }) {
    ...
    const preferencesIsOpen = useSelector(
        (state) => state?.user?.preferences?.isOpen
    );
            ...
            {isNoPreference ? (
                <div className="flex flex-col items-center justify-center h-[100vh]">
                    <h1 className="text-[20px] font-bold text-center">
                        You have not set your preference yet
                    </h1>
                    <p className="text-[14px] text-center">
                        Please set your preference to continue
                    </p>
                    <div className="mt-[40px] flex gap-4 flex-wrap justify-center p-4 max-w-[700px]">
                        {EstablishmentType.map((item, index) => {
                            const isSelected =
                                formik.values.preferences.includes(item);
                            return (
                                <div
                                    key={index}
                                    className={
                                        "py-[2em] px-[1.5em] rounded-lg bg-[white] shadow-lg border border-[black]/10 cursor-pointer " +
                                        (isSelected
                                    ...
                                    onClick={() => {
                                        const isSelected =
                                            formik.values.preferences.includes(
                                                item
                                            );
                                        if (!isSelected) {
                                            ...
                                            formik.setFieldValue(
                                                "preferences",
                                                formik.values.preferences.filter(
                                                    (p) => p !== item
                                                )
                                            );
                                        }
