...
import {
    ...
    FETCH_FAVORITE_ESTABLISHMENTS,
    fetchFavoriteEstablishmentsSuccess,
    fetchFavoriteEstablishmentsFailure,
...
function* fetchFavoriteEstablishmentsSaga(props) {
    try {
        const response = yield call(getFavoriteByUser, props.payload.userId);
        yield put(fetchFavoriteEstablishmentsSuccess(response.data));
    } catch (error) {
        yield put(fetchFavoriteEstablishmentsFailure(error));
    }
}
...
function* watchFetchFavoriteEstablishments() {
    yield takeLatest(
        FETCH_FAVORITE_ESTABLISHMENTS,
        fetchFavoriteEstablishmentsSaga
    );
}
...
export default function* establishmentsSaga() {
    yield all([
        ...
        fork(watchFetchFavoriteEstablishments),
    ]);
}
